#!/usr/bin/env python3
"""
Working UCI MTB Calendar Scraper - REFERENCE IMPLEMENTATION
==========================================================

⚠️  SPECULATIVE CODE - MAY BE NEEDED IN FUTURE ⚠️

This is a preserved reference implementation of the web scraping approach
that was successfully tested with saved HTML but ultimately replaced by
Excel parsing due to dynamic content loading issues.
...
SUCCESS: Parsed 158 events from saved HTML, proving the parsing logic works

This code demonstrates the successful parsing patterns for UCI calendar
structure and may be valuable if web scraping needs to be re-implemented
in the future with JavaScript rendering (Selenium/Playwright).

Key parsing methods:
- parse_competition_cards(): Extracts from carousel/featured events
- parse_calendar_items(): Extracts from main event listing  
- parse_dates(): Handles UCI date formats ("01 Jun 2025", "30 May - 01 Jun 2025")
...
def parse_saved_html():
    ...
    saved_html = Path(__file__).parent / "claude-input" / "Mountain Bike calendar _ UCI.html"
...
def main():
    """Test the working scraper with saved HTML - REFERENCE ONLY"""
    print("🚵‍♂️ Working UCI MTB Calendar Scraper - REFERENCE IMPLEMENTATION")
    print("⚠️  This is preserved reference code - use Excel parsing in production")
    print("=" * 80)
    ...
    print(f"\n🎯 Reference test complete! Found {len(standard_events)} events with valid dates")
    print("📖 See webscraping_approach_documentation.md for full implementation context")
    return standard_events
